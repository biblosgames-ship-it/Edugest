function testSlots() {
  const startT = 480;
  const endT = 720;
  const bStart = 570; // 09:30
  const bEnd = 600; // 10:00
  
  let preCount = 2; // if bStart < 10:00 -> preCount = 2
  let postCount = 3;
  
  const preDuration = Math.round((bStart - startT) / preCount); // (570-480)/2 = 45
  const postDuration = Math.round((endT - bEnd) / postCount); // (720-600)/3 = 40

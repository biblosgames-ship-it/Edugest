...
export const FacilityDashboard = ({ userData }: { userData: any }) => {
  ...
  const isAdminOrSupervisor = ['admin', 'coordinator', 'supervisor', 'superAdmin'].includes(userData?.role || '');
  ...
  const handleSaveIncident = async (formData: any) => {
    try {
      const centerId = profile?.center_id || userData?.center_id;
      if (!centerId) throw new Error("No se encontró el ID del centro.");
      const newIncident = await facilityService.createIncident({ ...formData, center_id: centerId });
  ...
  const handleSaveInventory = async (formData: any) => {
    try {
      const centerId = profile?.center_id || userData?.center_id;
      if (!centerId) throw new Error("No se encontró el ID del centro.");
      const newItem = await facilityService.createInventoryItem({ ...formData, center_id: centerId });
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-6 md:p-8 min-h-[500px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 text-slate-400 gap-4">
            ...
            <p className="font-bold uppercase tracking-widest text-xs">Cargando datos del plantel...</p>
          </div>
        ) : (
            ...
            {activeTab === 'tasks' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  {isAdminOrSupervisor && (
                    <button onClick={() => setShowTaskModal(true)} className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-indigo-700 transition-colors shadow-lg">
                ...
                {displayTasks.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {displayTasks.map(task => (
                      <div key={task.id} className="p-6 border border-slate-200 rounded-[2rem] bg-white shadow-sm flex flex-col justify-between hover:shadow-lg transition-shadow">
                        <div>
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg ${
                              task.status === 'pendiente' ? 'bg-amber-50 text-amber-600' :
                          ...
                          <h3 className="font-black text-slate-800 text-xl mb-1">{getAreaName(task.area_id)}</h3>
                          <p className="text-sm font-bold text-slate-500 mb-2 capitalize">Frecuencia: {task.frequency}</p>
                          {task.assignee_name && (
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-4">Asignado a: {task.assignee_name}</p>
                          )}
                        </div>
                        
                        {task.status === 'pendiente' && (
                          <button onClick={() => completeTask(task.id)} className="w-full py-3.5 mt-4 bg-emerald-500 text-white rounded-xl font-black uppercase text-[11px] tracking-widest hover:bg-emerald-600 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20">
            ...
            {activeTab === 'incidents' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => setShowIncidentModal(true)} className="flex items-center gap-2 px-5 py-2.5 bg-rose-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-rose-700 transition-colors shadow-lg shadow-rose-600/20">
                    <AlertTriangle size={16} /> Reportar Daño
                  </button>
                </div>
                ...
                {incidents.length === 0 ? (
                  <div className="text-center py-12 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 font-bold text-lg">No hay incidencias reportadas actualmente.</p>
                    <p className="text-slate-400">Todo el plantel está en orden.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {incidents.map(inc => (
                      <div key={inc.id} className="p-6 border border-slate-200 rounded-[2rem] bg-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex gap-4 items-start">
                           <div className={`p-4 rounded-2xl ${inc.status === 'resuelto' ? 'bg-emerald-100 text-emerald-600' : inc.urgency === 'alta' ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-600'}`}>
                             {inc.status === 'resuelto' ? <CheckCircle size={24} /> : <AlertTriangle size={24} />}
                           </div>
                           <div>
                             <div className="flex gap-2 items-center mb-1">
                               <h3 className="font-black text-slate-800 text-xl">{getAreaName(inc.area_id)}</h3>
                               <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg ${inc.status === 'resuelto' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
                                 {inc.status}
                               </span>
                             </div>
                             ...
                             <div className="flex gap-3 mt-2 text-xs font-black uppercase tracking-widest text-slate-400">
                               <span className="capitalize text-indigo-500">{inc.incident_type}</span>
                               <span>•</span>
                               <span className={inc.urgency === 'alta' ? 'text-rose-500' : 'text-amber-500'}>Urgencia: {inc.urgency}</span>
                             </div>
                           </div>
                        </div>

                        {inc.status !== 'resuelto' && isAdminOrSupervisor && (
                          <button onClick={() => resolveIncident(inc.id)} className="w-full md:w-auto px-6 py-3 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-emerald-100 transition-colors flex items-center justify-center gap-2">
              ...
              <div>
                ...
                {areas.length === 0 ? (
                  <div className="text-center py-12 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 font-bold">Configura las aulas, baños y áreas del plantel.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                     {areas.map(a => (
                       <div key={a.id} className="p-6 border border-slate-200 rounded-[2rem] bg-white shadow-sm flex flex-col justify-between hover:border-indigo-200 transition-colors">
                          <div>
                            <div className="flex justify-between items-start mb-4">
                              <span className="text-xs font-black uppercase px-3 py-1.5 bg-slate-100 text-slate-600 rounded-xl">{a.code}</span>
                              <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg ${a.status === 'bueno' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>{a.status}</span>
                            </div>
                            ...
                            {a.assignee_name && (
                              <div className="mt-4 p-3 bg-indigo-50 rounded-xl">
                                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-400 mb-1">Responsable</p>
              ...
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  {(isAdminOrSupervisor || isSupportStaff) && (
                    <button onClick={() => setShowInventoryModal(true)} className="flex items-center gap-2 px-5 py-2.5 bg-amber-500 text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-amber-600 transition-colors shadow-lg">
                ...
                {inventory.length === 0 ? (
                  <div className="text-center py-12 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 font-bold">No hay materiales de limpieza registrados.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                     {inventory.map(item => (
                       <div key={item.id} className={`p-6 border rounded-[2rem] shadow-sm text-center ${item.quantity <= item.min_stock ? 'bg-rose-50 border-rose-200' : 'bg-white border-slate-200'}`}>
                         <h3 className="font-black text-slate-800 text-lg mb-4">{item.name}</h3>
                         <div className="flex flex-col items-center justify-center">
                           <span className={`text-5xl font-black tracking-tighter ${item.quantity <= item.min_stock ? 'text-rose-600' : 'text-slate-700'}`}>
                             {item.quantity}
                           </span>
                           <span className="text-xs font-black uppercase tracking-widest text-slate-400 mt-2">{item.unit}</span>
                         </div>
                         {item.quantity <= item.min_stock && (
                           <p className="text-xs font-bold text-rose-500 mt-4 bg-rose-100 py-1.5 px-3 rounded-lg inline-block">¡Stock Bajo!</p>
                         )}
                       </div>
                     ))}
                  </div>
            ...
            {activeTab === 'metrics' && isAdminOrSupervisor && (
              <div>
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <h2 className="text-2xl font-black text-slate-800 tracking-tighter">Métricas de Plantel</h2>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-emerald-50 border border-emerald-100 p-8 rounded-[2.5rem]">
                    <h3 className="text-emerald-800 font-black uppercase tracking-widest text-[10px] mb-2">Tareas Completadas</h3>
                    <p className="text-5xl font-black text-emerald-600 tracking-tighter">
                      {tasks.filter(t => t.status === 'completada' || t.status === 'aprobada').length}
                  ...
                  <div className="bg-rose-50 border border-rose-100 p-8 rounded-[2.5rem]">
                    <h3 className="text-rose-800 font-black uppercase tracking-widest text-[10px] mb-2">Incidencias Activas</h3>
                  ...
                  <div className="bg-amber-50 border border-amber-100 p-8 rounded-[2.5rem]">
                    <h3 className="text-amber-800 font-black uppercase tracking-widest text-[10px] mb-2">Alertas de Inventario</h3>
      ...
      />
      <TaskModal isOpen={showTaskModal} onClose={() => setShowTaskModal(false)} onSave={handleSaveTask} areas={areas} staff={state.teachers} />
      <IncidentModal isOpen={showIncidentModal} onClose={() => setShowIncidentModal(false)} onSave={handleSaveIncident} areas={areas} />
      <InventoryModal isOpen={showInventoryModal} onClose={() => setShowInventoryModal(false)} onSave={handleSaveInventory} />
    </div>
  );
};

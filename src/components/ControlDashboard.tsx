...
export const ControlDashboard = () => {
  ...
  const teacherData = useMemo(() => {
    ...
    const teacherSchedule = todaySchedule
      ...
      .map(e => {
        ...
        return { ...e, tb, sub, course, room, isNow, startMinutes: start, endMinutes: end, sTime, eTime };
      })
      .sort((a, b) => a.startMinutes - b.startMinutes);
    ...
    const availableBlocks = state.timeBlocks.filter(
      (tb) => normalize(tb.day) === normalize(currentDay) && !busyBlocks.includes(tb.id)
    ).sort((a, b) => getMinutes(a.startTime || a.start_time) - getMinutes(b.startTime || b.start_time));
  ...
  const freeTeachers = useMemo(() => {
    const busyTeacherIds = new Set(activeClassesNow.map(c => c.teacher_id || c.teacherId).filter(Boolean));
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      <SEO title="Modo Control" description="Monitoreo en tiempo real de la actividad escolar por curso y docente." />
      ...
      <div className="bg-white rounded-[2rem] p-6 text-slate-900 relative overflow-hidden border-2 border-brand-blue shadow-lg">
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black tracking-tighter uppercase text-slate-900 leading-none">Torre de Control</h2>
              <p className="text-slate-500 font-black uppercase text-[8px] tracking-[0.2em] flex items-center gap-2 mt-1">
                <Calendar size={12} className="text-brand-blue" /> {currentDay} <span className="text-slate-300">|</span> <Clock size={12} className="text-brand-blue" /> {currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <div className="bg-emerald-50 border-2 border-emerald-100 px-6 py-3 rounded-2xl text-center shadow-sm">
              <p className="text-[8px] font-black uppercase text-emerald-700 tracking-[0.1em] mb-0.5">Clases en Vivo</p>
            ...
            <div className="bg-slate-50 border-2 border-slate-100 px-6 py-3 rounded-2xl text-center shadow-sm">
              <p className="text-[8px] font-black uppercase text-slate-500 tracking-[0.1em] mb-0.5">Personal Libre</p>
          ...
          <div className="flex flex-col gap-2">
             <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200">
               {['Matutina', 'Vespertina', 'Todas'].map((t: any) => (
                 <button
                   key={t}
                   onClick={() => { setActiveTanda(t); setSelectedId(''); }}
             ...
             <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200">
               <button
                 onClick={() => { setMode('course'); setSelectedId(''); }}
               ...
               <button
                 onClick={() => { setMode('teacher'); setSelectedId(''); }}
      ...
      <div className="max-w-xl mx-auto w-full">
        <div className="relative group">
          ...
          >
            <option value="">{mode === 'course' ? 'SELECCIONAR CURSO...' : 'SELECCIONAR DOCENTE...'}</option>
            {mode === 'course' 
              ? state.courses.map((c) => (
                  <option key={c.id} value={c.id}>{c.level} {c.grade} {c.section} ({c.tanda})</option>
                ))
              : state.teachers
          ...
          {courseTab === 'live' ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl">
                  <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-50">
                    <h3 className="text-lg font-black uppercase tracking-tighter text-slate-900 flex items-center gap-3">
                      <Activity className="text-brand-blue" size={20} /> {courseData.course.level} {courseData.course.grade} {courseData.course.section}
                    </h3>
                  ...
                  <div className="space-y-3">
                    {courseData.classes.length === 0 ? (
                      <div className="py-10 text-center text-slate-400 font-bold italic bg-slate-50 rounded-2xl text-[10px]">No hay clases programadas hoy.</div>
                    ) : (
                      courseData.classes.map((c) => (
                        <div key={c.id} className={`group relative p-5 rounded-2xl border-2 transition-all ${c.isNow ? 'bg-emerald-50 border-emerald-400 shadow-md' : 'bg-white border-slate-100 hover:border-slate-300'}`}>
                          {c.isNow && (
                          ...
                          <div className="flex justify-between items-center gap-4">
                            <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xs shadow-md ${c.isNow ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white'}`}>
                                {c.sTime}
                              </div>
                              <div>
              ...
              <div className="space-y-4">
                <div className="bg-brand-blue p-6 rounded-[2rem] text-white shadow-xl relative overflow-hidden group">
                  <h4 className="text-[8px] font-black uppercase tracking-widest opacity-70 mb-2">Próxima Clase</h4>
            ...
            <div className="space-y-4">
              <div className="bg-brand-blue/5 text-brand-blue border border-brand-blue/15 px-5 py-4 rounded-[1.5rem] text-xs font-bold flex items-center gap-3">
                ...
                <span>Estás previsualizando la pantalla de este curso tal como la verían sus alumnos y padres.</span>
              </div>
        ...
        <div className="space-y-6">
          ...
          {teacherTab === 'live' ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl">
                  <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-50">
                    <h3 className="text-lg font-black uppercase tracking-tight text-slate-800 flex items-center gap-3">
                      <User className="text-brand-blue" size={20} /> Agenda de Hoy: {teacherData.teacher.name}
                    </h3>
                  ...
                  <div className="space-y-3">
                    {teacherData.teacherSchedule.length === 0 ? (
                      <div className="py-10 text-center text-slate-400 italic text-[10px]">El docente no tiene clases programadas para hoy.</div>
                    ) : (
                      teacherData.teacherSchedule.map((c) => (
                        <div key={c.id} className={`relative p-5 rounded-2xl border transition-all ${c.isNow ? 'bg-indigo-50 border-indigo-200 shadow-md' : 'bg-white border-slate-50 hover:bg-slate-50'}`}>
                          {c.isNow && (
                          ...
                          <div className="flex justify-between items-center gap-4">
                            <div className="flex items-center gap-4">
                              <div className="text-center min-w-[50px]">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{c.sTime}</p>
                                <div className={`w-0.5 h-4 mx-auto my-0.5 rounded-full ${c.isNow ? 'bg-indigo-500' : 'bg-slate-200'}`}></div>
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{c.eTime}</p>
                              </div>
                              <div>
                                <p className={`text-sm font-black tracking-tight ${c.isNow ? 'text-indigo-900' : 'text-slate-800'}`}>{c.sub?.name}</p>
                                <p className="text-[9px] font-bold text-slate-400 flex items-center gap-2 uppercase tracking-widest mt-0.5">
                                  <BookOpen size={10} /> {c.course?.level} {c.course?.grade}{c.course?.section}
                                </p>
                              </div>
                            </div>
                            {c.room && (
              ...
              <div className="space-y-4">
                <div className={`p-6 rounded-[2rem] text-white shadow-xl transition-all duration-500 ${teacherData.currentClass ? 'bg-emerald-600' : 'bg-amber-500'}`}>
                  <h4 className="text-[8px] font-black uppercase tracking-widest opacity-80 mb-3">Estado Actual</h4>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                      {teacherData.currentClass ? <Activity size={20} className="animate-pulse" /> : <Clock size={20} />}
                    </div>
                    <div>
                      <p className="text-lg font-black uppercase leading-none">{teacherData.currentClass ? 'Ocupado' : 'Disponible'}</p>
                      <p className="text-[9px] font-bold opacity-70 mt-1">
                        {teacherData.currentClass ? `En ${teacherData.currentClass.course?.grade}${teacherData.currentClass.course?.section}` : 'Sin clases ahora'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl">
                  <h4 className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-4">Huecos Libres</h4>
                  <div className="flex flex-wrap gap-2">
                    {teacherData.availableBlocks.filter(tb => getMinutes(tb.startTime || tb.start_time) >= currentTimeMinutes).length === 0 ? (
                      <p className="text-[10px] font-bold text-slate-400 italic">No hay más hoy</p>
                    ) : (
                      teacherData.availableBlocks.filter(tb => getMinutes(tb.startTime || tb.start_time) >= currentTimeMinutes).slice(0, 3).map((tb) => (
                        <div key={tb.id} className="px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl flex flex-col items-center min-w-[70px]">
                          <span className="text-[9px] font-black text-slate-900">{tb.startTime || tb.start_time}</span>
                          <span className="text-[7px] font-bold text-slate-400 uppercase tracking-widest">Libre</span>
                        </div>
                      ))
                    )}
                  </div>
            ...
            <div className="space-y-4">
              <div className="bg-brand-blue/5 text-brand-blue border border-brand-blue/15 px-5 py-4 rounded-[1.5rem] text-xs font-bold flex items-center gap-3">
                <Info size={18} className="shrink-0" />
                <span>Estás previsualizando la pantalla de este docente tal como la vería él en su perfil de Edugest.</span>
              </div>
              <div className="bg-white p-2 rounded-[2.5rem] border border-slate-150 shadow-2xl overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              {activeClassesNow.length === 0 ? (
                <div className="bg-white p-24 rounded-[3.5rem] border-4 border-dashed border-slate-200 shadow-2xl text-center flex flex-col items-center">
                  ...
                  <h4 className="text-2xl font-black text-slate-900 uppercase mb-3">Sin actividad en este momento</h4>
                  <p className="text-slate-600 text-sm font-bold max-w-sm">No hay clases programadas para esta hora según el horario escolar vigente.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {activeClassesNow.map((c: any) => (
                    <div key={c.id} className="bg-white p-8 rounded-[3rem] border-2 border-slate-200 shadow-2xl hover:border-brand-blue transition-all group relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-6 mb-6">
                        ...
                        <div>
                          <p className="text-xs font-black text-emerald-600 uppercase tracking-[0.2em] mb-1">DANDO CLASE</p>
                          <div className="flex items-center gap-2 text-slate-900 font-black text-xs uppercase">
                             <Clock size={14} className="text-brand-blue" /> {c.tb?.startTime} - {c.tb?.endTime}
                          </div>
                        </div>
                      </div>
                      
                      <h4 className="text-2xl font-black text-black uppercase leading-none mb-6 min-h-[3rem] tracking-tighter">{c.sub?.name}</h4>
                      ...
                      <div className="space-y-4 pt-6 border-t-2 border-slate-50">
                        <div className="flex items-center gap-4">
                          ...
                          <p className="text-sm font-black text-slate-900 uppercase">{c.course?.level} {c.course?.grade} {c.course?.section}</p>
                        </div>
                        <div className="flex items-center gap-4">
                          ...
                          <p className="text-sm font-black text-slate-900 uppercase">{c.tea?.name}</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-slate-200/50 p-8 rounded-[3.5rem] border-2 border-slate-200 space-y-4 max-h-[700px] overflow-y-auto custom-scrollbar shadow-inner">
                {todaySchedule
                  .filter(e => getMinutes(state.timeBlocks.find(b => b.id === e.timeBlockId)?.startTime || '') > currentTimeMinutes)
                  .sort((a, b) => getMinutes(state.timeBlocks.find(b => b.id === a.timeBlockId)?.startTime || '') - getMinutes(state.timeBlocks.find(b => b.id === b.timeBlockId)?.startTime || ''))
                  .slice(0, 15)
                  .map((e: any) => {
                    ...
                    return (
                      <div key={e.id} className="bg-white p-6 rounded-2xl border-2 border-slate-300 flex items-center justify-between shadow-md hover:border-brand-blue transition-all">
                        <div className="flex items-center gap-5">
                           ...
                           <div>
                            <p className="text-sm font-black text-slate-900 uppercase leading-none mb-1">{sub?.name}</p>
                ...
                {todaySchedule.filter(e => getMinutes(state.timeBlocks.find(b => b.id === e.timeBlockId)?.startTime || '') > currentTimeMinutes).length === 0 && (
                  <div className="text-center py-20 px-6">
                    ...
                    <p className="text-xs font-black text-slate-900 uppercase tracking-widest">JORNADA FINALIZADA</p>
                  </div>
                )}
              </div>
            </div>

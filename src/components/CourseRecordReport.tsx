...
export const CourseRecordReport: React.FC<ReportProps> = ({
  onClose,
  ...
  const courseData = useMemo(() => {
    ...
    const globalAverage =
      studentAverages.length > 0 ? Math.round(totalGlobalAvg / studentAverages.length) : 0;

    const subjectChartData = Object.keys(subjectMetrics)
      .map((subId) => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              Grado: {courseData?.course?.level} {courseData?.course?.grade}{' '}
              {courseData?.course?.section ? `- Sección ${courseData?.course?.section}` : ''}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center text-center mb-10 pb-6 border-b-2 border-slate-800">
          {center?.logo_url && (
            <img
              src={center.logo_url}
              alt="Logo del Centro"
              className="w-20 h-20 object-contain mb-3 grayscale print:grayscale-0"
            />
          )}
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">
            {center?.name || 'Centro Educativo'}
          </h1>
          <h2 className="text-sm font-black text-slate-600 uppercase tracking-widest mt-1">
            Récord Académico de Grado
          </h2>
          <h3 className="text-xl font-bold text-indigo-700 mt-2">
            {courseData?.course?.level} {courseData?.course?.grade}{' '}
            {courseData?.course?.section ? `(Sección ${courseData?.course?.section})` : ''}
          </h3>
            ...
            <div className="mb-10 break-inside-avoid">
              ...
              <div className="grid grid-cols-3 gap-6 print:gap-4">
                ...
                <div className="col-span-1 border border-slate-200 rounded-xl p-4 bg-slate-50 print:bg-white">
                  <h3 className="text-[10px] font-black uppercase text-slate-500 mb-2">
                  ...
                  <div className="h-[140px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={Object.entries(courseData.rankingDist)
                          ...
                          dataKey="value"
                        >
                          {Object.entries(courseData.rankingDist)
                            .filter(([_, v]) => (v as number) > 0)
                            .map((entry, index) => (
                        ...
                        >
                          <div className="flex items-center gap-1.5">
                            <span
                              ...
                              style={{ backgroundColor: rankingColors[name] }}
                            ></span>
                          ...
                          <span className="text-slate-900">
                            ...
                            <span className="text-slate-400 font-normal">
                              ({Math.round(((value as number) / courseData.evalStudents) * 100)}%)
                            </span>
                          </span>
                        </div>
                      ))}
                ...
                <div className="col-span-1 border border-slate-200 rounded-xl p-4 bg-slate-50 print:bg-white flex flex-col justify-between">
                  ...
                  <div className="flex-1 h-[180px] min-h-0 mt-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        layout="vertical"
                        data={courseData.subjectChartData}
                      ...
                      >
                        ...
                        <Bar dataKey="promedio" fill="#4f46e5" radius={[0, 4, 4, 0]} barSize={12}>
                          <LabelList
                            dataKey="promedio"
                            position="right"
                            style={{ fontSize: '8px', fontWeight: 'bold', fill: '#4f46e5' }}
                          />
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                ...
                <div className="col-span-1 border border-slate-200 rounded-xl p-4 bg-slate-50 print:bg-white flex flex-col justify-between">
                  <div>
                    <h3 className="text-[10px] font-black uppercase text-slate-500 mb-4 text-center">
                      Promedio de Competencias
                    </h3>
                    <div className="flex justify-around items-end h-[60px] mb-4">
                      <div className="flex flex-col items-center gap-1">
                        <div
                          className="w-6 bg-indigo-200 rounded-t-sm flex items-end justify-center pb-0.5"
                          style={{ height: `${courseData.competencies.c1}%` }}
                        >
                          <span className="text-[7px] font-black text-indigo-900">
                            {courseData.competencies.c1}
                          </span>
                        </div>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <div
                          className="w-6 bg-indigo-400 rounded-t-sm flex items-end justify-center pb-0.5"
                          style={{ height: `${courseData.competencies.c2}%` }}
                        >
                          <span className="text-[7px] font-black text-white">
                            {courseData.competencies.c2}
                          </span>
                        </div>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <div
                          className="w-6 bg-indigo-600 rounded-t-sm flex items-end justify-center pb-0.5"
                          style={{ height: `${courseData.competencies.c3}%` }}
                        >
                          <span className="text-[7px] font-black text-white">
                            {courseData.competencies.c3}
                          </span>
                        </div>
                      ...
                      {(courseData.competencies.c4 > 0 || courseData.isSecondary) && (
                        <div className="flex flex-col items-center gap-1">
                          <div
                            className="w-6 bg-indigo-800 rounded-t-sm flex items-end justify-center pb-0.5"
                            style={{ height: `${courseData.competencies.c4}%` }}
                          >
                            <span className="text-[7px] font-black text-white">
                              {courseData.competencies.c4}
                            </span>
                          </div>
                  ...
                  <div className="border-t border-slate-200 pt-3">
                    <div className="bg-rose-50 text-rose-700 rounded-lg p-2.5 flex justify-between items-center text-[10px] font-black uppercase">
                      ...
                      <span className="text-lg">
                        {courseData.evalStudents - courseData.pendingSubjectsCount['Ninguna']}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b-2 border-slate-300">
                      <th className="py-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-50">
                        #
                      </th>
                      <th className="py-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-50">
                        Estudiante
                      </th>
                      <th className="py-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-50 text-center">
                        Promedio
                      </th>
                      <th className="py-2 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-50 text-center">
                        Pendientes
                      ...
                      >
                        ...
                        <td className="py-1.5 px-4 text-center">
                          ...
                          <span className="block text-[8px] font-bold text-slate-400 uppercase">
                            {student.avg > 0 ? student.ranking : 'N/A'}
                          </span>
                        </td>
                        <td className="py-1.5 px-4 text-center">
                          {student.pendingCount === 0 ? (
                        ...
                        <td className="py-1.5 px-4">
                          <span
                            className={`inline-block border px-3 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${student.riskColor}`}
                          >
                            {student.risk}
                          </span>
                        </td>
        ...
        <div className="mt-20 pt-10 grid grid-cols-3 gap-8 text-center break-inside-avoid">
          <div>
            ...
            <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
              {center?.director_name || 'Director/a'}
            </p>
          ...
          <div>
            <div className="border-b border-slate-400 mx-auto w-40 mb-2"></div>
            <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
              {center?.secretary_name || 'Secretario/a'}
            </p>
            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">
              Secretaría Docente
            </p>

...
export const HonorRollReport: React.FC<ReportProps> = ({ onClose, period: initialPeriod }) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[99999] bg-slate-50 block overflow-y-auto">
      ...
      <div
        id="honor-roll-container"
        className="max-w-6xl mx-auto p-10 space-y-12 pb-20 bg-slate-50"
      >
        ...
        <div className="relative pt-8 pb-16 px-10 bg-gradient-to-b from-amber-500 to-amber-600 rounded-[2.5rem] shadow-lg overflow-hidden">
          ...
          <div className="text-center mb-12 relative z-10">
            ...
            <p className="text-amber-100 font-bold uppercase tracking-widest text-[10px]">
              Los promedios más altos de toda la institución
            </p>
          </div>
            ...
            {honorData.topGlobal[1] && (
              <div className="flex flex-col items-center gap-3">
                <div className="w-16 h-16 md:w-24 md:h-24 bg-slate-200 rounded-full border-4 border-slate-300 flex items-center justify-center shadow-lg relative">
                  ...
                  <div className="absolute -bottom-2 bg-slate-400 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase">
                    2do
                  </div>
                </div>
                <div className="text-center">
                  <p className="text-white font-black text-lg truncate max-w-[150px]">
                    {honorData.topGlobal[1].name}
                  </p>
                  <p className="text-amber-100 text-[10px] font-bold uppercase">
                    {honorData.topGlobal[1].courseName}
                  </p>
                  <div className="mt-2 text-2xl font-black text-white">
                    {honorData.topGlobal[1].avg}
                  </div>
                </div>
              </div>
            )}
            ...
            {honorData.topGlobal[0] && (
              <div className="flex flex-col items-center gap-4 mb-8">
                <div className="w-24 h-24 md:w-32 md:h-32 bg-amber-100 rounded-full border-8 border-amber-300 flex items-center justify-center ring-[12px] ring-white/20 relative">
                  ...
                  <div className="absolute -bottom-3 bg-amber-500 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase shadow-xl ring-4 ring-amber-300">
                    Ganador
                  </div>
                </div>
                <div className="text-center">
                  <p className="text-white font-black text-xl drop-shadow-md">
                    {honorData.topGlobal[0].name}
                  </p>
                  <p className="text-amber-100 text-[10px] font-bold uppercase tracking-widest">
                    {honorData.topGlobal[0].courseName}
                  </p>
            ...
            {honorData.topGlobal[2] && (
              <div className="flex flex-col items-center gap-3">
                <div className="w-16 h-16 md:w-24 md:h-24 bg-orange-100 rounded-full border-4 border-orange-300 flex items-center justify-center shadow-lg relative">
                  ...
                  <div className="absolute -bottom-2 bg-orange-400 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase">
                    3ro
                  </div>
                </div>
                <div className="text-center">
                  <p className="text-white font-black text-lg truncate max-w-[150px]">
                    {honorData.topGlobal[2].name}
                  </p>
                  <p className="text-amber-100 text-[10px] font-bold uppercase">
                    {honorData.topGlobal[2].courseName}
                  </p>
                  <div className="mt-2 text-2xl font-black text-white">
                    {honorData.topGlobal[2].avg}
                  </div>
                </div>
              </div>
            )}
            ...
            <div key={level} className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">
                    Mérito por Nivel: {level}
                  </h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    Listado de excelencia académica
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {honorData.levels[level].map((student, i) => (
                  <div
                    key={i}
                    className="bg-white p-4 rounded-[1.5rem] border border-slate-200 shadow-sm hover:translate-y-[-2px] transition-all duration-300 group"
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex flex-col items-end">
                        <span className="text-2xl font-black text-slate-800 group-hover:text-amber-500 transition-colors">
                          {student.avg}
                        </span>
                    ...
                    <div>
                      <p className="text-sm font-black text-slate-800 uppercase tracking-tight leading-tight mb-1">
                        {student.name}
                      </p>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-50 flex items-center gap-1.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          key={s}
                          size={10}
                          fill={student.avg >= 95 ? '#f59e0b' : '#d1d5db'}
                          className={student.avg >= 95 ? 'text-amber-500' : 'text-slate-300'}
                        />
                      ))}
                    </div>
                  </div>
        ...
        <div className="bg-slate-900 rounded-[3rem] p-10 text-white overflow-hidden relative">
          ...
          <h3 className="text-2xl font-black uppercase tracking-tight mb-8 relative z-10">
            Consolidado por Cursos
          </h3>
              ...
              <div key={courseKey} className="space-y-4">
                ...
                <div className="space-y-3">
                  {honorData.coursesGroup[courseKey].map((s, i) => (
                    <div
                      key={i}
                      className="flex justify-between items-center bg-white/5 p-3 rounded-2xl hover:bg-white/10 transition-colors"
                    >

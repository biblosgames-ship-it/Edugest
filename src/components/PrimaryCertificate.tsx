...
const PrimaryCertificate: React.FC<PrimaryCertificateProps> = ({ studentId, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    issueDate: new Date().toLocaleDateString('es-DO', { day: 'numeric', month: 'long', year: 'numeric' }),
    issueDay: '10',
    issueMonth: 'Marzo',
    issueYear: '2026',
    directorName: '',
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const centerData = center || (profile?.center_id ? await dataService.getCenter(profile.center_id) : null);

      const today = new Date();
      // Extraer el año de término del año escolar seleccionado (ej: "2025-2026" -> "2026")
      const endingYear = selectedYear && selectedYear.includes('-') 
      ...
      const baseData = {
        ...
        schoolYear: selectedYear || '2025-2026',
        issuePlace: centerData?.municipality ? `${centerData.municipality}, R.D.` : 'Higüey, La Altagracia',
        issueDay: today.getDate().toString(),
        issueMonth: today.toLocaleString('es-DO', { month: 'long' }).replace(/^\w/, (c) => c.toUpperCase()),
      ...
      try {
        ...
        setFormData(prev => ({
          ...prev,
          ...baseData,
          studentName: `${student.names || ''} ${student.first_surname || ''} ${student.second_surname || ''}`.trim().toUpperCase(),
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`Distrito Educativo: ${formData.district}  |  Regional: ${formData.regional}`, centerX, 46, { align: 'center' });
    ...
    doc.setFontSize(12);
    doc.text(`ID de estudiante No. ${formData.rne} Inscrito(a) en el Centro Educativo`, centerX, 108, { align: 'center' });
    ...
    doc.setFont('times', 'normal');
    doc.setFontSize(11);
    doc.text(`Código del centro educativo: ${formData.centerCode} ,  Tanda / Sección: ${formData.tanda} / ${formData.section}`, centerX, 126, { align: 'center' });

    // 8. Párrafo de Aprobación (Espaciado mejorado)
    doc.setFontSize(12);
    doc.text(`Distrito Educativo No: ${formData.district} de: ${formData.municipality}, Regional No.: ${formData.regional} de: ${formData.municipality}`, centerX, 138, { align: 'center' });
    doc.text('quien cursó y aprobó los estudios correspondientes al 6to. grado del Nivel Primario,', centerX, 145, { align: 'center' });
    ...
    doc.setFont('times', 'normal');
    doc.setFontSize(12);
    doc.text(`Se expide la presente Certificación en _____${formData.municipality}______, República Dominicana, a los ${formData.issueDay}`, centerX, 168, { align: 'center' });
    doc.text(`días del mes de ${formData.issueMonth} del año______${formData.issueYear}______.`, centerX, 175, { align: 'center' });
    ...
    doc.setFont('times', 'normal');
    doc.text(`Directora del Distrito Educativo ${formData.district}, ${formData.municipality}`, centerX, footerY + 45, { align: 'center' });
  ...
  if (isLoading) {
    return (
      <div className="p-20 flex flex-col items-center justify-center gap-4 bg-white rounded-[2rem] shadow-2xl">
        ...
        <p className="text-slate-400 font-black uppercase text-xs tracking-widest">Preparando certificación...</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-300 max-h-[90vh] flex flex-col">
      ...
      <div className="p-6 bg-slate-900 text-white flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-black uppercase tracking-tight">Configurar Certificación</h3>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Nivel Primario - 6to Grado</p>
          </div>
        </div>
        <button 
          onClick={onClose}
      ...
      <div className="p-8 overflow-y-auto flex-1 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 text-indigo-600 mb-2">
            <Edit3 size={18} />
            <span className="text-[10px] font-black uppercase tracking-widest">Datos del Documento</span>
          </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Información del Estudiante</span>
              </div>
              
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Nombre completo del Alumno</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Código del SIGERD</label>
                  <input 
                    type="text" 
                    value={formData.rne}
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Año Académico</label>
                  <input 
                    type="text" 
            ...
            <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Información del Centro</span>
              </div>

              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Nombre del Centro Educativo</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Código</label>
                  <input 
                    type="text" 
                ...
                <div className="col-span-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Tanda</label>
                  <input 
                    type="text" 
                ...
                <div className="col-span-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Sección</label>
                  <input 
                    type="text" 
            ...
            <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha de Expedición</span>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Día</label>
                  <input 
                    type="text" 
            ...
            <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Autoridades Firmantes</span>
              </div>

              <div className="grid grid-cols-1 gap-4">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Director(a) del Centro</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Encargado(a) de Certificaciones (Distrito)</label>
                  <input 
                    type="text" 
                    value={formData.certificationOfficerName}
                    onChange={e => setFormData({...formData, certificationOfficerName: e.target.value})}
                    className="w-full px-5 py-3 bg-white border border-slate-200 rounded-2xl font-black text-xs text-slate-700"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Director(a) Distrital</label>
                  <input 
                    type="text" 
                    value={formData.districtDirectorName}
                    onChange={e => setFormData({...formData, districtDirectorName: e.target.value})}
                    className="w-full px-5 py-3 bg-white border border-slate-200 rounded-2xl font-black text-xs text-slate-700"
                  />
                </div>
              </div>
        ...
        <div className="bg-slate-50 rounded-[3rem] p-8 border border-slate-100 flex flex-col items-center justify-center relative group overflow-hidden">
          ...
          <div className="w-full max-w-[360px] aspect-[1/1.41] bg-white shadow-2xl border border-slate-200 rounded-lg p-7 flex flex-col items-center text-center scale-95 group-hover:scale-100 transition-transform duration-500 overflow-y-auto relative custom-scrollbar">
            ...
            <div className="w-full px-2 mb-2">
              <p className="text-[14px] font-black text-indigo-600 uppercase italic leading-none truncate">{formData.studentName || 'NOMBRE DEL ALUMNO'}</p>
              <div className="w-full h-[0.75px] bg-slate-400 mt-1"></div>
              <p className="text-[6px] text-slate-400 mt-1 uppercase tracking-wider">nombre y apellidos del estudiante</p>
            </div>

            <div className="mt-4 space-y-1">
              <p className="text-[7px] text-slate-600">ID de estudiante No. <span className="font-bold">{formData.rne || '_______'}</span></p>
              <p className="text-[7px] text-slate-600">Inscrito(a) en el Centro Educativo</p>
              <p className="text-[9px] font-black text-slate-900 uppercase tracking-wide">{formData.schoolName}</p>
            </div>
            ...
            <div className="mt-4 px-4 space-y-1">
              <p className="text-[7px] text-slate-500 text-center leading-[10px]">
                Distrito Educativo No: {formData.district} de: {formData.municipality}, Regional No.: {formData.regional} de: {formData.municipality} quien cursó y aprobó los estudios correspondientes al 6to. grado del Nivel Primario, en el año académico {formData.schoolYear}
              </p>
            </div>

            <div className="mt-6 px-3">
              <p className="text-[7px] text-slate-600 text-center leading-[11px] italic">
                Se expide la presente Certificación en _____{formData.municipality}______, República Dominicana, a los {formData.issueDay} días del mes de {formData.issueMonth} del año______{formData.issueYear}______.
              </p>
            </div>
            
            <div className="grid grid-cols-2 gap-4 w-full mt-auto pt-10">
              <div className="border-t border-slate-200 pt-1">
                <p className="text-[4px] font-black text-slate-800 truncate">{formData.directorName || 'Director(a)'}</p>
                <p className="text-[3px] text-slate-400 uppercase">Director Centro</p>
              </div>
              <div className="border-t border-slate-200 pt-1">
                <p className="text-[4px] font-black text-slate-800 truncate">{formData.districtDirectorName || 'Distrital'}</p>
      ...
      <div className="bg-amber-50 p-6 flex items-center gap-5 border-t border-amber-100 flex-shrink-0">
        ...
        <div>
          <p className="text-[10px] font-black text-amber-800 uppercase tracking-widest mb-1">Aviso de Seguridad</p>
          <p className="text-[9px] font-bold text-amber-700/80 leading-relaxed max-w-2xl">
            Asegúrese de que el nombre coincida exactamente con el acta de nacimiento. Una vez impreso, este documento tiene validez oficial ante el MINERD.
          </p>
        </div>
      </div>
    </div>

...
export const FinanceReports = () => {
  ...
  const reportData = useMemo(() => {
    ...
    const totalPaid = invoices.filter(i => i.status === 'paid').reduce((acc, i) => acc + Number(i.amount_final), 0);
    ...
    const courseStats = courses.map(course => {
      ...
      courseStudents.forEach(student => {
        ...
        const sPaid = studentInvoices.filter(i => i.status === 'paid').reduce((acc, i) => acc + Number(i.amount_final), 0);
        const sBalance = sDebt - sPaid;
        const hasMora = studentInvoices.some(i => i.status === 'overdue' || (i.status === 'pending' && new Date(i.due_date) < new Date()));
  ...
  if (selectedCourseId) {
    ...
    const courseStudents = state.students.filter(s => s.course_id === selectedCourseId)
      .map(student => {
        const studentInvoices = invoices.filter(i => i.student_id === student.id);
        const sDebt = studentInvoices.reduce((acc, i) => acc + Number(i.amount_final), 0);
        const sPaid = studentInvoices.filter(i => i.status === 'paid').reduce((acc, i) => acc + Number(i.amount_final), 0);
        ...
        <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-8 border-b border-slate-100 pb-8">
            <div className="flex items-center gap-4">
              {center?.logo_url && <img src={center.logo_url} alt="Logo" className="w-16 h-16 object-contain" />}
              <div>
                <h2 className="text-xl font-black uppercase tracking-tighter text-slate-900">{center?.name || 'EDUGEST SCHOOL'}</h2>
                <h3 className="text-sm font-bold text-indigo-600 uppercase">REPORTE DE COBROS: {course?.name}</h3>
              </div>
            </div>
            <div className="flex flex-col items-end gap-2">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Fecha: {new Date().toLocaleDateString()}</p>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {courseStudents.map((s, idx) => (
                <tr key={s.id} className="text-[11px] font-bold text-slate-700">
                  ...
                  <td className="px-4 py-2 uppercase">{s.first_surname} {s.names}</td>
                  <td className="px-4 py-2 text-center">
                    <span className={`uppercase text-[9px] px-2 py-0.5 rounded ${s.hasMora ? 'bg-rose-100 text-rose-700' : s.balance > 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                      {s.hasMora ? 'Mora' : s.balance > 0 ? 'Pendiente' : 'Al Día'}
                    </span>
                  </td>
            ...
            <tfoot>
              <tr className="border-t-2 border-slate-900 font-black text-slate-900 uppercase">
                <td colSpan={3} className="px-4 py-4 text-right text-[10px]">Total Pendiente del Grado:</td>
                <td className="px-4 py-4 text-right text-sm">RD$ {course?.debt.toLocaleString()}</td>
              </tr>
            </tfoot>
          </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-2 bg-slate-900 p-8 rounded-[2.5rem] text-white flex flex-col justify-between shadow-2xl shadow-slate-900/20">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Estado General de Cobros</p>
          </div>
          <div className="mt-8 flex items-end justify-between">
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Por Cobrar (Pendiente)</p>
              <p className="text-3xl font-black">RD$ {reportData.pendingBalance.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          ...
          <p className="text-xl font-black text-slate-900">RD$ {reportData.totalPaid.toLocaleString()}</p>
        </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">En Mora</p>
      ...
      <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Desglose detallado de deudas y estados</p>
          </div>
                ...
                <tr key={course.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-8 py-4">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-black text-slate-700 uppercase tracking-tight">{course.name}</span>
                      <button 
                  ...
                  <td className="px-8 py-4 text-center">
                    <div className={`flex items-center justify-center gap-1.5 font-black text-[10px] ${course.moraCount > 0 ? 'text-rose-600' : 'text-slate-300'}`}>
                      <Clock size={12} /> {course.moraCount}
                    </div>
                  </td>
                  <td className={`px-8 py-4 text-right font-black text-sm ${course.debt > 0 ? 'text-rose-600' : 'text-slate-900'}`}>

...
export const PayrollManager = () => {
  ...
  const [concepts, setConcepts] = useState<any[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: 'tss', name: 'TSS (Seguro Vejez)', type: 'deduction', valueType: 'percent', value: 2.87 },
      { id: 'sfs', name: 'SFS (Seguro Salud)', type: 'deduction', valueType: 'percent', value: 3.04 },
    ];
  });
  ...
  const toggleConceptForEmployee = (empId: string, conceptId: string) => {
    setMasterPayroll(masterPayroll.map(m => {
      if (m.id === empId) {
  ...
  const calculateNet = (emp: any) => {
    ...
    emp.appliedConcepts.forEach((cId: string) => {
      ...
      const value = concept.valueType === 'percent' 
        ? (base * (concept.value / 100)) 
        : Number(concept.value);
  ...
  const handleProcessMonth = () => {
    ...
    localStorage.setItem('edugens_ledger_entries', JSON.stringify([...newEntries, ...currentEntries]));
  ...
  const exportToPDF = () => {
    ...
    doc.text(`REPORTE DE NÓMINA INSTITUCIONAL - ${monthName.toUpperCase()} ${new Date().getFullYear()}`, 148, 26, { align: 'center' });
    doc.text(`RNC: ${center?.rnc || '000-00000-0'} | Tel: ${center?.phone || '809-000-0000'}`, 148, 31, { align: 'center' });
  ...
  const exportToExcel = () => {
    const data = masterPayroll.map(emp => {
      ...
      concepts.forEach(c => {
        ...
        const val = isApplied ? (c.valueType === 'percent' ? (calcs.bruto * (c.value / 100)) : c.value) : 0;
        row[c.name] = val;
      });
      ...
      {activeTab === 'master' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
              <h3 className="text-[10px] font-black uppercase text-slate-400 mb-6 tracking-widest">Añadir a Nómina</h3>
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {availableStaff.map((emp: any) => (
                  ...
                  >
                    <p className="text-[10px] font-black text-slate-900 uppercase mb-1">{emp.full_name}</p>
          ...
          <div className="lg:col-span-9 space-y-6">
            <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm min-h-[600px]">
              <div className="flex items-center justify-between mb-10">
                <div>
                  <h2 className="text-xl font-black uppercase tracking-tighter">Nómina Fija Institucional</h2>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Configuración permanente de pagos</p>
                </div>
                ...
                <table className="w-full text-left border-separate border-spacing-y-3">
                  ...
                  <tbody>
                    {masterPayroll.map((emp: any) => {
                      ...
                      return (
                        <tr key={emp.id} className="bg-slate-50/50 hover:bg-white hover:shadow-xl hover:shadow-slate-100/50 transition-all group">
                          <td className="py-6 pl-6 rounded-l-[2rem]">
                            <p className="text-[11px] font-black text-slate-900 uppercase mb-1">{emp.full_name}</p>
                            <p className="text-[8px] font-bold text-slate-400 uppercase">{emp.role}</p>
                          </td>
                          <td className="py-6">
                            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-slate-100 w-32">
                              ...
                              <input 
                                ...
                                onChange={(e) => updateMasterEntry(emp.id, 'baseSalary', e.target.value)}
                          ...
                          <td className="py-6 text-right pr-6">
                            <p className="text-[12px] font-black text-slate-900">RD$ {calculations.net.toLocaleString()}</p>
                            <p className="text-[8px] font-bold text-rose-500 uppercase">Deduc: RD$ {calculations.deductions.toFixed(2)}</p>
                          </td>
                          <td className="py-6 rounded-r-[2rem] pr-6 text-right">
                            <button onClick={() => removeFromMaster(emp.id)} className="p-2 text-slate-300 hover:text-rose-500 transition-all">
                              <Trash2 size={16} />
                            </button>
                          </td>
                        </tr>
              ...
              {masterPayroll.length === 0 && (
                <div className="py-32 text-center text-slate-300">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">Configura tu nómina maestra añadiendo personal</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-xl font-black uppercase tracking-tighter">Conceptos Globales</h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Define TSS, Seguros, Bonos y Retenciones</p>
              </div>
              <button 
                onClick={() => setConcepts([...concepts, { id: Date.now().toString(), name: 'Nuevo Concepto', type: 'deduction', valueType: 'percent', value: 0 }])}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {concepts.map((concept: any, idx: number) => (
                <div key={concept.id} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 relative group">
                  <button 
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-[8px] font-black uppercase text-slate-400 mb-2 block">Nombre del Concepto</label>
                      <input 
                        type="text"
                        value={concept.name}
                        onChange={(e) => {
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-[8px] font-black uppercase text-slate-400 mb-2 block">Tipo</label>
                        <select 
                          value={concept.type}
                          onChange={(e) => {
                            const updated = [...concepts];
                      ...
                      <div>
                        <label className="text-[8px] font-black uppercase text-slate-400 mb-2 block">Cálculo por</label>
                    ...
                    <div>
                      <label className="text-[8px] font-black uppercase text-slate-400 mb-2 block">Valor ({concept.valueType === 'percent' ? '%' : 'RD$'})</label>
                      <input 
                        type="number"
                        value={concept.value}
                        onChange={(e) => {

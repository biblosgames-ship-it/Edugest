...
export const LedgerManager = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tighter">Libro Contable Maestro</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Gestión integral de ingresos y egresos</p>
            </div>
          </div>
          
          <div className="flex bg-slate-50 p-1.5 rounded-[1.5rem] border border-slate-100">            <button 
      ...
      {activeTab === 'daily' && <DailyLedger entries={entries} setEntries={setEntries} categories={categories} />}
      {activeTab === 'config' && <AccountsConfig categories={categories} setCategories={setCategories} />}
    </div>
  );
};
    ...
    <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm animate-fade-in relative">
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Define tus conceptos de ingreso y egreso</p>
        </div>
        <button onClick={() => setShowModal(true)} className="flex items-center gap-3 bg-indigo-600 text-white px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-xl shadow-indigo-100 hover:scale-105 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.map((cat: any) => (
          <div key={cat.id} className="p-8 rounded-[2.5rem] border border-slate-100 bg-slate-50/30 relative">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => deleteCategory(cat.id)} className="p-2 text-slate-400 hover:text-rose-600"><Trash2 size={14} /></button>
              </div>
            </div>
            <h4 className="text-lg font-black text-slate-900 mb-4 uppercase tracking-tighter">{cat.name}</h4>
            <div className="space-y-2">
              {cat.items.map((item: any, idx: number) => (
                <div key={idx} className="flex justify-between items-center p-3 bg-white rounded-xl border border-slate-100">
                  ...
                  <span className="text-[10px] font-black text-slate-900">RD$ {item.price.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-xl rounded-[3rem] p-10 shadow-2xl">
            ...
            <div className="space-y-6">
              <input type="text" value={newCatName} onChange={(e) => setNewCatName(e.target.value)} placeholder="Nombre Cuenta" className="w-full px-6 py-4 bg-slate-50 rounded-2xl" />
              <div className="flex gap-2">
                <button onClick={() => setNewCatType('income')} className={`flex-1 py-4 rounded-2xl text-[10px] font-black ${newCatType === 'income' ? 'bg-emerald-500 text-white' : 'bg-slate-50'}`}>INGRESOS</button>
                <button onClick={() => setNewCatType('expense')} className={`flex-1 py-4 rounded-2xl text-[10px] font-black ${newCatType === 'expense' ? 'bg-rose-500 text-white' : 'bg-slate-50'}`}>EGRESOS</button>
              </div>
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="text-[10px] font-black uppercase text-slate-400">Items</label>
                  <button onClick={addItem} className="text-indigo-600 text-[10px] font-black">+ AÑADIR</button>
                </div>
                {newItems.map((item, idx) => (
                  <div key={idx} className="flex gap-2 mb-2">
                    <input type="text" value={item.name} onChange={(e) => updateItem(idx, 'name', e.target.value)} className="flex-1 px-4 py-2 bg-slate-50 rounded-lg text-xs" placeholder="Nombre" />
                    <input type="number" value={item.price} onChange={(e) => updateItem(idx, 'price', Number(e.target.value))} className="w-24 px-4 py-2 bg-slate-50 rounded-lg text-xs" placeholder="Precio" />
                    <button onClick={() => removeItem(idx)}><Trash2 size={14} className="text-rose-500" /></button>
                  </div>
                ))}
              </div>
              <div className="flex gap-4">
                <button onClick={resetModal} className="flex-1 py-4 bg-slate-100 rounded-2xl text-xs font-black">CANCELAR</button>
                <button onClick={handleSaveCategory} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl text-xs font-black">GUARDAR</button>
              </div>
            </div>
          </div>
        </div>
...
const DailyLedger = ({ entries, setEntries, categories }: any) => {
  ...
  const filteredEntries = useMemo(() => {
    return entries.filter((e: any) => {
      ...
      const matchesSearch = e.desc.toLowerCase().includes(searchTerm.toLowerCase()) || 
  ...
  const handleAddToCart = () => {
    ...
    setAmount(0); setQuantity(1); setDiscount(0); setSelectedItem(null);
  };
  ...
  const handlePrintReceipt = (entry: any) => {
    ...
    doc.text(`Tel: ${center?.phone || '809-000-0000'} | RNC: ${center?.rnc || '000-00000-0'}`, 50, 24, { align: 'center' });
  ...
  const handleCashClosing = () => {
    ...
    doc.text(`EFECTIVO EN CAJA: RD$ ${todayEntries.reduce((acc, e) => acc + (e.type === 'income' ? e.amount : -e.amount), 0).toLocaleString()}`, 196, finalY + 10, { align: 'right' });
  ...
  const handleCondensedCashClosing = () => {
    const doc = new jsPDF({ format: [100, 150] });
    if (filteredEntries.length === 0) return toast.error('No hay movimientos en este rango de fechas');

    doc.setFontSize(14); doc.setFont("helvetica", "bold");
    doc.text(center?.name || 'EDUGEST SCHOOL', 50, 15, { align: 'center' });
    doc.setFontSize(8); doc.text("RESUMEN CONTABLE CONDENSADO", 50, 22, { align: 'center' });
    doc.setFontSize(7); doc.setFont("helvetica", "normal");
    ...
    doc.setFont("helvetica", "normal");
    const methodNames: any = { cash: 'Efectivo', transfer: 'Transferencia', card: 'Tarjeta', check: 'Cheque' };
    ...
    const grandTotal = filteredEntries.reduce((acc, e) => acc + (e.type === 'income' ? e.amount : -e.amount), 0);
    doc.text(`BALANCE: RD$ ${grandTotal.toLocaleString()}`, 50, currentY, { align: 'center' });

    currentY += 25;
    doc.line(15, currentY, 45, currentY);
    doc.setFontSize(6); doc.text("CONTABILIDAD", 22, currentY + 4);
  ...
  const handleExportPDF = () => {
    ...
    Object.keys(grouped).forEach(accountName => {
      ...
      const accountTotal = accountEntries.reduce((acc: any, e: any) => acc + (e.type === 'income' ? e.amount : -e.amount), 0);

      autoTable(doc, {
        startY: currentY,
        head: [[{ content: `CUENTA: ${accountName}`, colSpan: 4, styles: { fillColor: [79, 70, 229] } }]],
        body: accountEntries.map((e: any) => [e.date, e.item, e.type === 'income' ? 'ING' : 'EGR', `RD$ ${e.amount.toLocaleString()}`]),
    ...
    <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm animate-fade-in relative">
      <div className="flex flex-col gap-6 mb-10">
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="flex gap-2">
            <button onClick={() => { setType('income'); setShowModal(true); }} className="flex items-center gap-3 bg-emerald-500 text-white px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-xl shadow-emerald-100">
              <TrendingUp size={18} /> Ingreso
            </button>
            <button onClick={() => { setType('expense'); setShowModal(true); }} className="flex items-center gap-3 bg-rose-500 text-white px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-xl shadow-rose-100">
          ...
          <div className="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-[2rem] border border-slate-100">
            <div className="flex items-center gap-2 px-4 border-r border-slate-200">
              ...
              <input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-transparent border-none text-[10px] font-black uppercase focus:ring-0 w-32" />
            </div>
            <div className="flex items-center gap-2 px-4 border-r border-slate-200">
              <Calendar size={16} className="text-slate-400" />
              <div className="flex items-center gap-1">
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="bg-transparent border-none text-[9px] font-black focus:ring-0 p-0" />
                <span className="text-[9px] font-black text-slate-300">A</span>
                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="bg-transparent border-none text-[9px] font-black focus:ring-0 p-0" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-indigo-50/50 p-6 rounded-[2.5rem] border border-indigo-100/50 flex flex-wrap items-center justify-between gap-6">
          <div className="flex flex-wrap items-center gap-6">
            <div className="space-y-2">
              <label className="text-[9px] font-black text-indigo-400 uppercase tracking-widest px-2">Tipo de Reporte</label>
              <select 
                value={reportType} 
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-indigo-400 uppercase tracking-widest px-2">Método de Pago</label>
              <select 
                value={methodFilter} 
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black text-indigo-400 uppercase tracking-widest px-2">Filtrar Cuenta</label>
              <select 
                value={accountFilter} 
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredEntries.map((entry: any) => (
              <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase">{entry.desc}</td>
                <td className="px-8 py-5 text-right font-black text-emerald-600 text-sm">{entry.type === 'income' ? `RD$ ${entry.amount.toLocaleString()}` : '-'}</td>
                <td className="px-8 py-5 text-right font-black text-rose-600 text-sm">{entry.type === 'expense' ? `RD$ ${entry.amount.toLocaleString()}` : '-'}</td>
                <td className="px-8 py-5 text-right">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => handlePrintReceipt(entry)} className="p-2 text-slate-400 hover:text-indigo-600"><Printer size={16} /></button>
                    <button onClick={() => setEntries(entries.filter((e: any) => e.id !== entry.id))} className="p-2 text-slate-300 hover:text-rose-500"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 sm:p-6">
          <div className="bg-white w-full max-w-xl rounded-[3rem] p-8 sm:p-10 shadow-2xl animate-in zoom-in-95 duration-200 max-h-[95vh] flex flex-col overflow-hidden">
            <div className="flex justify-between items-center mb-8 shrink-0">
              <h3 className="text-xl font-black uppercase tracking-tighter">Registrar {type === 'income' ? 'Ingreso' : 'Egreso'}</h3>
              <button onClick={() => { setShowModal(false); setCart([]); }} className="p-2 hover:bg-slate-50 rounded-full text-slate-400"><X size={24} /></button>
            </div>
            ...
            <div className="space-y-6 overflow-y-auto pr-2 custom-scrollbar flex-1">
              ...
              {selectedCat && (
                ...
                >
                  ...
                  {selectedCat.items.map((item: any, i: number) => (
                    <option key={i} value={item.name}>{item.name} (RD$ {item.price})</option>
                  ))}
                </select>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block">Cantidad</label>
                  <input type="number" min="1" value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-indigo-600" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block">Descuento (RD$)</label>
                  <input type="number" min="0" value={discount} onChange={(e) => setDiscount(Number(e.target.value))} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-indigo-600" />
                </div>
              </div>

              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block">Precio Unitario (RD$)</label>
                <div className="flex gap-2">
                  <input type="number" value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="flex-1 px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-indigo-600" />
                  <button 
              ...
              <div className="space-y-3 border-y border-slate-100 py-4">
                {cart.length === 0 && <p className="text-center text-[10px] text-slate-300 py-4 uppercase font-bold">La canasta está vacía</p>}
                {cart.map((item) => (
                  <div key={item.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 group">
                    <div>
                      ...
                      <p className="text-[9px] font-bold text-slate-400 uppercase">Unit: RD$ {item.price.toLocaleString()} {item.discount > 0 && `(-RD$ ${item.discount})`}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <p className="text-xs font-black text-slate-900">RD$ {item.total.toLocaleString()}</p>
              ...
              <div className="bg-slate-900 p-6 rounded-[2rem] flex justify-between items-center text-white shadow-xl shrink-0">
                ...
                <div className="text-right">
                  <p className="text-[9px] font-bold uppercase opacity-50">{cart.length} Products</p>
                </div>
              </div>

              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block">Nota / Comentario</label>
                <textarea value={desc} onChange={(e) => setDesc(e.target.value)} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-indigo-600 h-20" placeholder="Ej: Pago de uniformes..." />
              </div>
            </div>

  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const levelCourses = courses.filter(c => 
        (c.level || '').toString().toLowerCase().trim() === targetLevel
      );

      if (levelCourses.length === 0) {
        toast.error(`No hay cursos en el nivel "${editingLevel}" para actualizar.`, { id: loadingToast });
        return;
      }
      ...
      const { error } = await supabase
        ...
        .upsert(plansArray.map(p => ({ ...p, center_id: currentCenterId })), { onConflict: 'course_id' });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
          <div className="mb-10">
            <h2 className="text-2xl font-black uppercase tracking-tighter text-slate-900">Configuración por Niveles</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Define los costos generales para cada etapa académica</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {levels.map((level) => {
              ...
              return (
                ...
                >
                  <div className={`w-16 h-16 rounded-[1.8rem] flex items-center justify-center shadow-lg ${
                    hasConfig ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400'
                  }`}>
                    <CreditCard size={28} />
                  </div>
                  <div>
                    <h4 className="text-sm font-black text-slate-900 uppercase tracking-tighter">{level}</h4>
                    <p className={`text-[8px] font-black uppercase mt-1 ${hasConfig ? 'text-emerald-600' : 'text-slate-400'}`}>
          ...
          <div className="mt-12 p-6 bg-indigo-50/50 border border-indigo-100 rounded-[2rem] flex items-start gap-4">
            ...
            <p className="text-[10px] font-bold text-indigo-700 uppercase leading-relaxed tracking-widest">
              Al configurar un nivel, todos los cursos asociados (por ejemplo, desde 1ro hasta 6to de Secundaria) 
              adoptarán automáticamente estos costos. Esto asegura una facturación uniforme y rápida.
            </p>
          </div>
        </div>
      </div>
          ...
          {editingLevel ? (
            <form onSubmit={handleSave} className="space-y-8">
              <div className="flex items-center gap-4 mb-2">
                <div className="p-4 bg-indigo-500 rounded-3xl shadow-xl"><Settings2 size={24} /></div>
                <div>
              ...
              <div className="space-y-6">
                {selectedLevelCount === 0 && (
                  <div className="p-4 bg-rose-500/20 border border-rose-500/50 rounded-2xl flex items-start gap-3">
                    ...
                    <p className="text-[9px] font-black uppercase text-rose-500 leading-tight">
                      No se detectan cursos para este nivel. Por favor verifica los nombres en la sección de Cursos.
                    </p>
                  </div>
                )}
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">Inscripción (DOP)</label>
                  <div className="relative">
                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-indigo-400 font-black">RD$</span>
                    <input 
                      type="number"
                      required
                      value={formData.enrollment_fee}
                      onChange={(e) => setFormData({ ...formData, enrollment_fee: Number(e.target.value) })}
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">Cuota Mensual (DOP)</label>
                  <div className="relative">
                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-indigo-400 font-black">RD$</span>
                    <input 
                      type="number"
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">Cantidad de Meses</label>
                  <select 
                  ...
                  >
                    <option value={0} className="bg-slate-900">0 (Solo Inscripción)</option>
                    <option value={1} className="bg-slate-900">1 (Pago Único)</option>
                    <option value={10} className="bg-slate-900">10 (Año Escolar Std)</option>
                    <option value={12} className="bg-slate-900">12 (Año Completo)</option>
                  </select>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">Inicio Pago (Día)</label>
                    <input 
                      type="number"
                      min="1" max="31"
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">Límite Pago (Día)</label>
                    <input 
                      type="number"
                      min="1" max="31"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">Días de Gracia</label>
                    <input 
                      type="number"
                      min="0"
                      value={formData.grace_days}
                      onChange={(e) => setFormData({ ...formData, grace_days: Number(e.target.value) })}
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-xl font-black focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                    />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">% Mora</label>
                    <input 
                      type="number"
                      min="0"
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em] px-2">Mes de Inicio Escolar</label>
                  <select 
                  ...
                  >
                    {['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'].map((month, idx) => (
                      <option key={month} value={idx + 1} className="bg-slate-900">{month}</option>
                    ))}
                  </select>
                </div>
              </div>

  ...
  const dataByLevel = useMemo(() => {
    ...
    const calculateMetrics = (filterLevel: 'Primaria' | 'Secundaria' | 'General') => {
      ...
      const subjectChartData = Object.keys(subjectMetrics)
        ...
        .slice(0, 8); // top 8/bottom 8 o solo 8 materias principales
      ...
      <div className="mb-16 print:mb-12 break-inside-avoid">
        ...
        <div className="grid grid-cols-3 gap-6 print:gap-4">
                ...
                >
                  ...
                  <span className="text-slate-900">
                    ...
                    <span className="text-slate-400 font-normal">
                      ({Math.round(((r.value as number) / data.evalStudents) * 100)}%)
                    </span>
                  </span>
                </div>
              ))}
          ...
          <div className="col-span-1 border border-slate-200 rounded-xl p-4 bg-slate-50 print:bg-white flex flex-col justify-between">
            <h3 className="text-[10px] font-black uppercase text-slate-500 mb-2">
              Asignaturas Pendientes (&#60; 70)
            </h3>
            <div className="space-y-2 mt-2 flex-1">
              {Object.entries(data.pendingSubjectsCount).map(([k, v]: [string, any]) => (
                <div
                  key={k}
                  className="flex justify-between items-center text-[10px] border-b border-slate-200 pb-1.5 last:border-0"
                >
                  <span
                    className={`font-bold uppercase ${k === 'Ninguna' ? 'text-emerald-600' : 'text-slate-600'}`}
                  >
                    {k}
                  </span>
                  <div className="flex gap-4">
                    ...
                    <span className="text-slate-400 w-8 text-right font-medium">
                      {Math.round((v / data.totalStudents) * 100)}%
                    </span>
                  </div>
                </div>
              ))}
          ...
          <div className="col-span-1 border border-slate-200 rounded-xl p-4 bg-slate-50 print:bg-white flex flex-col justify-center">
            ...
            <div className="flex justify-around items-end h-[100px] mb-4">
              <div className="flex flex-col items-center gap-2">
                <div
                  className="w-8 bg-indigo-200 rounded-t-sm relative flex items-end justify-center pb-1"
                  style={{ height: `${data.competencies.c1}%` }}
                >
                  <span className="text-[8px] font-black text-indigo-900">
                    {data.competencies.c1}
                  </span>
                </div>
              ...
              <div className="flex flex-col items-center gap-2">
                <div
                  className="w-8 bg-indigo-400 rounded-t-sm relative flex items-end justify-center pb-1"
                  style={{ height: `${data.competencies.c2}%` }}
                >
              ...
              <div className="flex flex-col items-center gap-2">
                <div
                  className="w-8 bg-indigo-600 rounded-t-sm relative flex items-end justify-center pb-1"
                  style={{ height: `${data.competencies.c3}%` }}
                >
              ...
              {(data.competencies.c4 > 0 || title.includes('Secundari')) && (
                <div className="flex flex-col items-center gap-2">
                  <div
                    className="w-8 bg-indigo-800 rounded-t-sm relative flex items-end justify-center pb-1"
                    style={{ height: `${data.competencies.c4}%` }}
                  >
          ...
          <div className="col-span-3 border border-slate-200 rounded-xl p-4 pb-6 bg-slate-50 print:bg-white h-[220px]">
            <h3 className="text-[10px] font-black uppercase text-slate-500 mb-2">
              Promedio Institucional por Asignatura
            </h3>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
              ...
              >
                ...
                <Bar dataKey="promedio" fill="#3b82f6" radius={[0, 4, 4, 0]} barSize={12}>
                  <LabelList
                    dataKey="promedio"
                    position="right"
                    style={{ fontSize: '9px', fontWeight: 'bold', fill: '#0f172a' }}
                  />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-800 uppercase tracking-tight">
              Récord Académico Institucional
            </h1>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center text-center mb-10 pb-6 border-b-2 border-slate-800">
          ...
          <h2 className="text-sm font-black text-slate-600 uppercase tracking-widest mt-1">
            Récord Académico Institucional Consolidado
          </h2>
        ...
        <div className="mt-20 pt-10 grid grid-cols-4 gap-8 text-center break-inside-avoid">
          ...
          <div>
            <div className="border-b border-slate-400 mx-auto w-40 mb-2"></div>
            <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
              {center?.district_director_name || 'Director/a Distrital'}
            </p>
            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">
              Dirección Distrital
            </p>
          ...
          <div>
            <div className="w-24 h-24 border-2 border-slate-200 rounded-full mx-auto flex items-center justify-center opacity-50 mb-2">
              <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest rotate-[-30deg]">
                SELLO OFICIAL
              </span>
            </div>
            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">

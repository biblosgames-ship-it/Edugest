...
export const WorkloadReport: React.FC<ReportProps> = ({ onClose }) => {
  ...
  const downloadPDF = () => {
    ...
    doc.text('REPORTE DE CARGA HORARIA Y FUNCIONES DOCENTES', pageWidth / 2, 25, { align: 'center' });
    ...
    autoTable(doc, {
      startY: 40,
      head: [['Docente', 'Inicial (25h)', 'Primaria (25h)', 'Secundaria (30h)', 'Otros', 'Total h/s', 'Materias / Cursos']],
    ...
    <div className="fixed inset-0 z-[99999] bg-slate-50 block overflow-y-auto pb-20">
      <div className="bg-white px-6 py-4 border-b border-slate-200 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-800 uppercase tracking-tight">Carga Horaria y Funciones</h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Distribución de Horas Semanales por Nivel</p>
          </div>
        </div>

        <button onClick={downloadPDF} className="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg transition-all active:scale-95">
          <Download size={18} />
          Exportar Carga
        </button>
      </div>
           ...
           <table className="w-full text-left border-collapse">
              <thead>
                 <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Inicial (25h)</th>
                    <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Primaria (25h)</th>
                    <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Secundaria (30h)</th>
                    <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Total h/s</th>
                    <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Materias Asignadas</th>
                 </tr>
              </thead>
              <tbody>
                   ...
                   <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
                      <td className="p-6">
                         <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-black text-slate-800 uppercase tracking-tight">{t.name}</span>
                         </div>
                      </td>
                      <td className="p-6 text-center text-sm font-bold text-slate-500">{t.formattedLevels['Inicial']}</td>
                      <td className="p-6 text-center text-sm font-bold text-slate-500">{t.formattedLevels['Primaria']}</td>
                      <td className="p-6 text-center text-sm font-bold text-slate-500">{t.formattedLevels['Secundaria']}</td>
                      ...
                      <td className="p-6">
                         <div className="flex flex-wrap gap-2">
                            {t.subjects.map((s, si) => (
                              <span key={si} className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-[9px] font-bold uppercase whitespace-nowrap">
                                 {s}
                              </span>
                            ))}
                         </div>

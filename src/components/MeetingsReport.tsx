...
import { ArrowLeft, Download, Users, Calendar as CalendarIcon, Clock, FileText } from 'lucide-react';
...
export const MeetingsReport: React.FC<ReportProps> = ({ onClose }) => {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`Año Escolar: ${selectedYear} | Total Reuniones: ${meetings.length}`, pageWidth / 2, 37, { align: 'center' });
    ...
    <div className="fixed inset-0 z-[99999] bg-slate-50 block overflow-y-auto pb-20">
      <div className="bg-white px-6 py-4 border-b border-slate-200 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-800 uppercase tracking-tight">Agenda de Reuniones</h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Cronograma de Encuentros y Sesiones</p>
          </div>
        </div>

        <button onClick={downloadPDF} className="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg transition-all active:scale-95">
          <Download size={18} />
          Exportar Agenda
        </button>
      </div>
      ...
      <div className="max-w-5xl mx-auto p-8 space-y-6">
        {meetings.length === 0 ? (
          <div className="bg-white rounded-[3rem] p-20 text-center border border-slate-100 shadow-xl">
             ...
             <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight mb-2">No hay reuniones agendadas</h3>
             <p className="text-slate-400 text-sm max-w-xs mx-auto">Las reuniones programadas en el calendario administrativo aparecerán aquí.</p>
          </div>
        ) : (

...
const ConductBalanceCertificate: React.FC<ConductBalanceCertificateProps> = ({ studentId, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    observations: 'El estudiante ha demostrado respeto hacia sus compañeros, docentes y personal administrativo, manteniendo una actitud responsable, participativa y acorde con los valores cristianos promovidos por la institución.',
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const defaultAdminName = state.teachers?.find(t => t.role === 'management' || t.role === 'management_teacher' || t.role === 'administrative')?.name || 'Lic. Karina Gómez';

      const baseData = {
        issuePlace: centerData?.municipality ? `${centerData.municipality}, República Dominicana` : 'Santo Domingo, República Dominicana',
        issueDay: currentDay,
      ...
      try {
        ...
        if (student.family && student.family.length > 0) {
          const parents = student.family.filter((f: any) => f.relation?.toLowerCase().includes('madre') || f.relation?.toLowerCase().includes('padre'));
          if (parents.length > 0) {
        ...
        const courseDisplay = course ? `${course.grade} de ${course.level?.toLowerCase().includes('prim') ? 'Primaria' : course.level?.toLowerCase().includes('sec') ? 'Secundaria' : course.level}` : '';

        setFormData(prev => ({
          ...prev,
          ...baseData,
          studentName: `${student.names || ''} ${student.first_surname || ''} ${student.second_surname || ''}`.trim(),
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      doc.setFontSize(16);
      const safeCenterName = centerDisplay.name ? centerDisplay.name.toUpperCase() : 'CENTRO EDUCATIVO';
      doc.text(safeCenterName, centerX, 50, { align: 'center' });
      
      doc.setFont('times', 'normal');
      doc.setFontSize(11);
      doc.text(`Código: ${centerDisplay.code} | Tel: ${centerDisplay.phone}`, centerX, 57, { align: 'center' });
      ...
      doc.save(`Certificacion_Conducta_Saldo_${formData.studentName ? formData.studentName.replace(/\s+/g, '_') : 'Estudiante'}.pdf`);
    } catch (error: any) {
    ...
    <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-300 max-h-[90vh] flex flex-col print:h-screen print:max-h-screen print:rounded-none print:shadow-none print:border-none">
      ...
      <div className="p-6 bg-slate-900 text-white flex items-center justify-between flex-shrink-0 no-print">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Conducta y Saldo</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
           <button 
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            <div className="grid grid-cols-1 gap-5">
              ...
              <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Conducta</span>
                </div>
                  ...
                  {['Excelente', 'Muy Buena', 'Buena', 'Regular'].map(c => (
                    <label key={c} className="flex items-center gap-2 cursor-pointer">
                      <input 
                        ...
                        onChange={(e) => setFormData({...formData, conduct: e.target.value as any})}
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Observaciones</label>
                  <textarea 
              ...
              <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Estado de Saldo</span>
                </div>
                ...
                <div className="space-y-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      ...
                      onChange={(e) => setFormData({...formData, balanceStatus: 'al-dia', balanceConcept: 'No aplica.', balanceAmount: ''})}
                      className="text-indigo-600 focus:ring-indigo-500"
                    />
                    <span className="text-xs font-bold text-slate-700">Se encuentra al día con sus compromisos económicos.</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="radio" 
                      ...
                      onChange={(e) => setFormData({...formData, balanceStatus: 'pendiente', balanceConcept: ''})}
                      className="text-indigo-600 focus:ring-indigo-500"
                    />
                    <span className="text-xs font-bold text-slate-700">Presenta un balance pendiente.</span>
                  </label>
                </div>
                ...
                {formData.balanceStatus === 'pendiente' && (
                  <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-100">
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Monto (RD$)</label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        onChange={e => setFormData({...formData, balanceConcept: e.target.value})}
                        className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl font-bold text-xs text-slate-700"
               ...
               <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Firmantes</span>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Nombre Director/a</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Cargo</label>
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Nombre Adm./Caja</label>
                    <input 
                      type="text" 
        ...
        <div className="w-1/2 p-8 overflow-y-auto bg-slate-200/50 flex items-start justify-center custom-scrollbar">
          ...
          <div id="print-area" className="w-full max-w-[816px] min-h-[1056px] bg-[#ffffff] shadow-2xl p-16 flex flex-col font-serif relative text-[#000000]">
            
            {/* Header Centro */}
            <div className="text-center mb-8 pb-8 border-b-2 border-[#000000]">
              <h1 className="text-2xl font-bold uppercase mb-4 tracking-wide">{centerDisplay.name}</h1>
            ...
            <h2 className="text-2xl font-bold text-center uppercase tracking-widest mb-10">Certificación</h2>

            <p className="text-justify text-[15px] leading-loose mb-8">
              Quien suscribe, <strong>{formData.directorName}</strong>, en calidad de {formData.directorTitle} del {centerDisplay.name}, certifica que el estudiante:
            </p>

            {/* Datos del Estudiante (Párrafo fluido) */}
            <div className="mb-8 mt-2">
              <p className="text-justify text-[15px] leading-loose">
                <strong>{formData.studentName}</strong>, Código del Sigerd No.: <strong>{formData.rne}</strong> del Grado <strong>{formData.courseName}</strong>, Año Escolar: <strong>{formData.schoolYear}</strong>. Hijo(a) de los señores: <strong>{formData.tutorName}</strong>.
              </p>
            </div>

            <div className="mb-8 border-t border-[#d1d5db] pt-8">
              <h3 className="font-bold uppercase tracking-widest mb-4 border-b border-[#d1d5db] pb-1">Conducta</h3>
              <p className="text-[15px] mb-4">Durante su permanencia en esta institución, el estudiante ha mantenido una conducta:</p>
            ...
            <div className="mb-10 border-t border-[#d1d5db] pt-8">
              <h3 className="font-bold uppercase tracking-widest mb-4 border-b border-[#d1d5db] pb-1">Estado de Saldo</h3>
              <p className="text-[15px] mb-4">Luego de revisar los registros administrativos y financieros del centro, hacemos constar que el estudiante:</p>
              
              <div className="space-y-4 ml-4 mb-6">
                <div className="flex items-center gap-3">
                  {formData.balanceStatus === 'al-dia' ? <CheckSquare size={16} /> : <Square size={16} />}
                  <span className="text-[15px]">Se encuentra al día con sus compromisos económicos.</span>
                </div>
                <div className="flex items-center gap-3">
                  {formData.balanceStatus === 'pendiente' ? <CheckSquare size={16} /> : <Square size={16} />}
                  <span className="text-[15px]">Presenta un balance pendiente de: <strong>{formData.balanceStatus === 'pendiente' && formData.balanceAmount ? `RD$ ${formData.balanceAmount}` : '_______________________'}</strong></span>
                </div>
              </div>
            ...
            <p className="text-justify text-[15px] leading-loose mb-16">
              La presente certificación se expide a solicitud de la parte interesada, en la ciudad de {formData.issuePlace}, a los {formData.issueDay} días del mes de {formData.issueMonth} del año {formData.issueYear}.
            </p>
            ...
            <div className="mt-auto pt-10">
              ...
              <div className="mt-16 text-center">
                ...
                <p className="text-xs text-[#6b7280] uppercase tracking-widest">Sello del Centro Educativo</p>
              </div>
            </div>

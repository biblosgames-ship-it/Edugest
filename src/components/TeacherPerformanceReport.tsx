...
import { ArrowLeft, Download, User, BookOpen, TrendingUp, TrendingDown, Award, AlertCircle } from 'lucide-react';
...
export const TeacherPerformanceReport: React.FC<ReportProps> = ({ onClose, period: initialPeriod }) => {
  ...
  const performanceData = useMemo(() => {
    ...
    const teacherStats = teachers.map(teacher => {
      ...
      const subjectAverages = teacherAssignments.map(asg => {
        const asgGrades = grades.filter(g => 
        ...
        const avg = asgGrades.length > 0 
          ? Math.round(asgGrades.reduce((sum, g) => sum + (g.grade || 0), 0) / asgGrades.length)
          : 0;
  ...
  }, [state.teachers, state.assignments, state.courses, state.subjects, state.grades, selectedPeriod]);
  ...
  const downloadPDF = () => {
    ...
    doc.text(`Periodo: ${selectedPeriod} | Año Escolar: ${selectedYear}`, pageWidth / 2, 37, { align: 'center' });
    ...
    autoTable(doc, {
      startY: currentY,
      head: [['Pos.', 'Docente', 'Índice Académico']],
      body: [...performanceData.ranking].reverse().slice(0, 3).map((t, i) => [i + 1, t.name, t.totalAvg]),
      theme: 'striped',
    ...
    Object.keys(performanceData.levels).forEach(level => {
      ...
      performanceData.levels[level].forEach(teacher => {
        ...
        autoTable(doc, {
          startY: currentY,
          head: [[{ content: `Docente: ${teacher.name} (Promedio: ${teacher.totalAvg})`, colSpan: 3, styles: { fillColor: [79, 70, 229] } }], ['Materia', 'Curso', 'Índice']],
          body: teacherRows,
          theme: 'grid',
    ...
    <div className="fixed inset-0 z-[99999] bg-slate-50 block overflow-y-auto pb-20">
      <div className="bg-white px-6 py-4 border-b border-slate-200 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-800 uppercase tracking-tight">Desempeño por Docente</h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Análisis de Índices Académicos por Cátedra</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={downloadPDF} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg transition-all active:scale-95">
            <Download size={16} />
            Exportar Desempeño
          </button>
        </div>
      ...
      <div id="teacher-performance-container" className="max-w-7xl mx-auto p-8 space-y-10 bg-slate-50">
        
        {/* ENCABEZADO OFICIAL */}
        <div className="flex flex-col items-center justify-center text-center mb-10 pb-8 border-b-2 border-indigo-100">
          {center?.logo_url && (
            <img src={center.logo_url} alt="Logo del Centro" className="w-24 h-24 object-contain mb-4" />
          )}
          <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tight">{center?.name || 'Centro Educativo'}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 ...
                 {performanceData.ranking.slice(0, 3).map((t, i) => (
                   <div key={i} className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center font-black text-xs">{i+1}</div>
           ...
           <div className="bg-rose-50 border border-rose-100 p-6 rounded-[2rem] shadow-sm">
              ...
              <div className="space-y-3">
                 {[...performanceData.ranking].reverse().slice(0, 3).map((t, i) => (
                   <div key={i} className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center font-black text-xs">{i+1}</div>
                         <span className="text-sm font-bold text-slate-700">{t.name}</span>
                      </div>
                    ...
                    <div key={i} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                      <div className="p-5 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-black text-slate-800 uppercase tracking-tight leading-none">{teacher.name}</p>
                            <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">{teacher.subjectAverages.length} Materias</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-xl font-black text-indigo-600 leading-none">{teacher.totalAvg}</p>
                        ...
                        {teacher.subjectAverages.map((sub: any, si: number) => (
                          <div key={si} className="space-y-1.5">
                            <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-tight">
                              <span className="text-slate-500 truncate max-w-[140px]">{sub.subjectName} <span className="text-slate-300">({sub.courseName})</span></span>
                              <span className={sub.avg >= 90 ? 'text-emerald-500' : sub.avg >= 70 ? 'text-amber-500' : 'text-rose-500'}>
                                {sub.avg} pts
                              </span>
                            </div>

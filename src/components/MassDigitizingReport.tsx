...
export const MassDigitizingReport: React.FC<ReportProps> = ({ onClose, period: initialPeriod }) => {
  ...
  const digitizingData = useMemo(() => {
    ...
    courses.forEach((course: any) => {
      ...
      const courseGrades = grades.filter(
        (g: any) =>
          g.course_id === course.id && g.period?.toLowerCase() === selectedPeriod.toLowerCase()
      );
      ...
      const subjectsInCourseIds = Array.from(
        new Set(allCourseGrades.map((g: any) => g.subject_id))
      );
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(
      `Periodo: ${selectedPeriod} | Año Escolar: ${selectedYear} | Fecha: ${new Date().toLocaleDateString()}`,
      pageWidth / 2,
      32,
      { align: 'center' }
    );
    ...
    Object.keys(digitizingData).forEach((level) => {
      ...
      digitizingData[level].forEach((course: any) => {
        course.subjects.forEach((sub: any, idx: number) => {
          tableData.push([idx === 0 ? course.name : '', sub.name, sub.status]);
        });
      });
    ...
    <div className="fixed inset-0 z-[99999] bg-slate-100 block overflow-y-auto">
      ...
      <div className="bg-white px-6 py-4 border-b border-slate-200 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={downloadPDF}
            className="flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-rose-200 transition-all active:scale-95"
          >
            <Download size={16} />
            Exportar Auditoría
          </button>
          ...
          <div className="flex gap-4">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center min-w-[120px]">
              ...
              <span className="text-2xl font-black text-slate-800">
                {Object.values(digitizingData).flat().length}
              </span>
            </div>
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center min-w-[120px]">
              ...
              <span className="text-2xl font-black text-emerald-600">
                {
                  Object.values(digitizingData)
                    .flat()
                    .filter((c) => c.completed === c.total && c.total > 0).length
                }
              </span>
            </div>
          </div>
                  ...
                  {digitizingData[level].map((course: any) => (
                    ...
                    >
                      <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                        ...
                        <span
                          className={`text-[10px] font-black px-2 py-0.5 rounded-full uppercase ${course.completed === course.total ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}
                        >
                          ...
                          course.subjects.map((sub: any) => (
                            <div key={sub.id} className="flex justify-between items-center group">
                              ...
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-[9px] font-black uppercase ${sub.status === 'Digitado' ? 'text-emerald-500' : 'text-rose-400'}`}
                                >
                                  {sub.status}
                                </span>
                                {sub.status === 'Digitado' ? (

...
export const scheduleService = {
  ...
  fastTargetedFill: async (
    ...
    // 6. FASE 2: DESAGREGACIÓN FLEXIBLE DE BLOQUES DOBLES Y REUBICACIÓN EN CADENA
    for (const task of missingTasks) {
      ...
      // PASO A: Intentar Desagregación Flexible (Separar un bloque doble de otra materia en este curso para liberar el hueco)
      let resolvedBySplit = false;
      for (const day of days) {
        for (const slot of courseSlots) {
          if (slot.isBreak) continue;
          const sStartMins = toMins(slot.start);
          ...
          // Verificar si el docente de la tarea faltante está LIBRE en este día y hora
          ...
          // Si el docente está libre, ver qué clase ocupa actualmente este slot en el curso
          ...
          if (occupyingEntry && !occupyingEntry.is_locked && !(lockedKeys || []).includes(occupyingEntry.id)) {
            // Verificar si la materia ocupante tiene 2 horas hoy (es un bloque doble separable) o se puede mover a otro día
            for (const altDay of days) {
              if (altDay.toLowerCase() === day.toLowerCase()) continue;
              ...
              for (const altSlot of courseSlots) {
                ...
                // Comprobar si el slot alternativo en el curso está libre
                const altSlotOccupied = workingSchedule.some((e: any) => {
                  if (String(e.course_id) !== String(task.courseId)) return false;
                  if ((e.day || '').trim().toLowerCase() !== altDay.toLowerCase()) return false;
                  const eS = toMins(e.start_time);
                  const eE = toMins(e.end_time);
                ...
                if (altSlotOccupied) continue;

                // Comprobar si el docente de la materia ocupante está libre en altDay y altSlot
                const occTeacherBusy = occupyingEntry.teacher_id && workingSchedule.some((e: any) => {
                ...
                if (occTeacherBusy) continue;

                // ¡DESAGREGACIÓN / REUBICACIÓN FLEXIBLE EXITOSA!
                ...
                newEntriesToInsert.push(newEntry);
                placedCount++;
                resolvedBySplit = true;
                break;
              }
              if (resolvedBySplit) break;
            }
          }
          if (resolvedBySplit) break;
        }
      ...
      if (resolvedBySplit) continue;

      // PASO B: Reubicación en Cadena en Otros Cursos del Docente
      if (!task.teacherId) continue;
      for (const day of days) {
        ...
        for (const slot of courseSlots) {
          ...
          if (conflictingEntries.length === 1) {
            ...
            if (otherEntry.is_locked || (lockedKeys || []).includes(otherEntry.id)) continue;
  ...
  // CLONAR HORARIO DE UNA TANDA A OTRA (e.g. Matutina -> Vespertina)
  cloneShiftSchedule: async (
    state: any,
    profile: any,
    fromShift: string,
    toShift: string,
    ...
    const centerId = profile.center_id;
    const schoolYear = year;
    const { courses: allCourses, levelSchedules, breakPreferences } = state;

    // 1. Obtener entradas de la tanda origen
    ...
    if (fetchErr || !sourceEntries || sourceEntries.length === 0) {
      throw new Error(`No hay un horario generado en la tanda ${fromShift} para clonar.`);
    }
    ...
    // Helper para generar slots de cualquier curso en cualquier tanda
    const getSlots = (course: any, targetShift: string) => {
      const isMorning = targetShift.toLowerCase().substring(0, 3) === 'mat';
      const official = (levelSchedules || []).find(
        (ls: any) =>
          ls.level === course.level &&
          (ls.shift === (isMorning ? 'Matutina' : 'Vespertina') || !ls.shift)
      ...
      for (let i = 0; i < preCount; i++) {
        let e = i === preCount - 1 ? bStart : currPre + preDur;
        slots.push({ start: fromMins(currPre), end: fromMins(e) });
        currPre = e;
      }
      ...
      for (let i = 0; i < postCount; i++) {
        let e = i === postCount - 1 ? endT : currPost + postDur;
        slots.push({ start: fromMins(currPost), end: fromMins(e) });
        currPost = e;
      }
    ...
    // 2. Mapear cada entrada de la tanda origen a la tanda destino
    const newEntries: any[] = [];
    const fromCourses = (allCourses || []);

    const targetCourseMap = new Map<string, string>();
    fromCourses.forEach((fc: any) => {
      const matchingTargetCourse = fromCourses.find(
        (tc: any) =>
          tc.id !== fc.id &&
          tc.grade === fc.grade &&
          tc.section === fc.section &&
          tc.level === fc.level &&
          (tc.tanda || '').toLowerCase().includes(toShift.toLowerCase().substring(0, 3))
      );
      if (matchingTargetCourse) {
        targetCourseMap.set(fc.id, matchingTargetCourse.id);
      } else {
        targetCourseMap.set(fc.id, fc.id);
      }
    });

    for (const src of sourceEntries) {
      const courseObj = fromCourses.find((c: any) => c.id === src.course_id);
      if (!courseObj) continue;

      const srcSlots = getSlots(courseObj, fromShift);
      const dstSlots = getSlots(courseObj, toShift);

      let srcSlotIdx = -1;
      let minDiff = Infinity;
      const srcMins = toMins(src.start_time);

      srcSlots.forEach((slot, idx) => {
        const diff = Math.abs(toMins(slot.start) - srcMins);
        if (diff < minDiff) {
          minDiff = diff;
          srcSlotIdx = idx;
        }
      ...
      if (srcSlotIdx === -1 || srcSlotIdx >= dstSlots.length) {
        srcSlotIdx = Math.min(Math.max(0, srcSlotIdx), dstSlots.length - 1);
      }

      const dstSlot = dstSlots[srcSlotIdx];
      if (!dstSlot) continue;

      const targetCourseId = targetCourseMap.get(src.course_id) || src.course_id;
      ...
      newEntries.push({
        ...
        day: src.day,
        shift: toShift,
        start_time: dstSlot.start,
        end_time: dstSlot.end,
        school_year: schoolYear
      });
    }

    if (newEntries.length === 0) {
      throw new Error('No se pudieron proyectar las clases a los bloques de la tanda destino.');
    }

    // 3. Eliminar horario previo en la tanda destino
    ...
    if (delErr) throw new Error('Error al limpiar tanda destino: ' + delErr.message);

    // 4. Insertar las nuevas clases clonadas
    const chunkSize = 200;
    for (let i = 0; i < newEntries.length; i += chunkSize) {
      const chunk = newEntries.slice(i, i + chunkSize);
      const { error: insErr } = await supabase.from('schedule_entries').insert(chunk);
      if (insErr) throw new Error('Error al guardar horario clonado: ' + insErr.message);
    }

    return {
      clonedCount: newEntries.length,
      message: `✅ ¡Horario clonado exitosamente!\n\nSe copiaron ${newEntries.length} clases de la tanda ${fromShift} a la tanda ${toShift} con sus horas y bloques adaptados automáticamente.`
    };
  }
};

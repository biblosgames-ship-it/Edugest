  ...
  return (
    <div className="flex h-screen bg-brand-bg overflow-hidden transition-colors duration-300">
      ...
      <main className="flex-1 h-screen overflow-hidden relative">
        {/* Mobile menu trigger + Center Name Pill */}
        <div className="md:hidden fixed top-4 left-4 z-40 flex items-center gap-2.5 max-w-[calc(100vw-2rem)] pointer-events-none">
          <button
            onClick={() => setSidebarOpen(true)}
            className="pointer-events-auto bg-slate-900/80 backdrop-blur-md text-white p-3.5 rounded-2xl border border-white/10 shadow-xl hover:bg-slate-800 transition-all active:scale-95 cursor-pointer flex items-center justify-center relative shrink-0"
            title="Abrir Menú"
          >
            <Menu size={20} />
            {unreadCount > 0 && (
          ...
          <div className="pointer-events-auto bg-slate-900/80 backdrop-blur-md text-white px-3.5 py-3 rounded-2xl border border-white/10 shadow-xl flex items-center gap-2 overflow-hidden">
            {center?.logo_url ? (
              <img
                src={center.logo_url}
                alt="Centro"
                className="w-5 h-5 rounded-md object-contain bg-white/10 p-0.5 shrink-0"
              />
            ) : (
              <Building2 size={15} className="text-indigo-400 shrink-0" />
            )}
            <span className="text-[11px] font-black uppercase tracking-wider text-white truncate max-w-[170px] sm:max-w-xs">
